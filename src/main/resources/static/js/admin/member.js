...
export async function modifyMember(e) {
    ...
    if (thisId === "btn-modify") {
        arrInput.forEach((item, index) => {
            ...
            oldContents[index] = item.value; // 현재 값 저장
        });
    } else if (thisId === "btn-back") {
        arrInput.forEach((item, index) => {
            arrInput[index].readOnly = true;
            arrInput[index].value = oldContents[index]; // 저장해둔 값을 복원
        });
 ...
 * 회원을 삭제하는 함수
 *
 * 이 함수는 `data-` 속성을 기반으로 특정 회원을 삭제합니다.
 * 삭제할 대상의 ID와 유형을 식별하고, 서버에 삭제 요청을 전송하여 삭제 작업을 수행합니다.
 *
 * @param {HTMLElement} manager - 삭제 작업을 수행할 대상 요소. 이 요소는 `data-member-id`와 `data-member-type` 속성을 통해 삭제할 항목을 식별합니다.
 *
 */
